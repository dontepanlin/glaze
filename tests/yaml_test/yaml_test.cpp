...
arr = [1, 2, 3])");
   ...
   "read_basic_struct"_test = [] {
      std::string yaml_input = R"(i = 42
d = 2.71
hello = "Test String"
arr = [4, 5, 6])";

      my_struct s{};
      expect(not glz::read_yaml(s, yaml_input));
      expect(s.i == 42);
      expect(s.d == 2.71);
   ...
   "read_integer"_test = [] {
      std::string yaml_input = "123";
      int value{};
      expect(not glz::read_yaml(value, yaml_input));
      expect(value == 123);
   };

   "read_float"_test = [] {
      std::string yaml_input = "3.14159";
      double value{};
      expect(not glz::read_yaml(value, yaml_input));
   ...
   "read_string"_test = [] {
      std::string yaml_input = R"("Hello TOML")";
      std::string value{};
      expect(not glz::read_yaml(value, yaml_input));
...
value = 5.5)"};
      auto error = glz::read_yaml(sc, buffer);
...
y = "test1"
      ...
      expect(not glz::read_yaml(sc, buffer));
...
y = "test2"
      ...
      expect(not glz::read_yaml(ac, buffer));
   ...
   "read_advanced_nested_struct"_test = [] {
      ...
      expect(not glz::read_yaml(dac, buffer));
   ...
   "read_inline_table"_test = [] {
      std::string yaml_input = R"(name = "Test Person"
inline_data = { key1 = "value1", key2 = 100 })";
      struct_with_inline_table s{};
      auto error = glz::read_yaml(s, yaml_input);
      expect(!error) << glz::format_error(error, yaml_input);
...
b = "test string" # another eol comment
   ...
   "read_non_null_terminated"_test = [] {
      ...
      auto error = glz::read_yaml(s_val, toml_data);
      ...
      error = glz::read_yaml(s_val2, buffer_just_value);
