...
namespace glz
{
   ...
   struct parse<YAML>
   {
      ...
      {
         from<YAML, std::decay_t<T>>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx),
   ...
   // Parse YAML key (bare key or quoted key)
   template <class Ctx, class It, class End>
   ...
   struct from<YAML, T>
   {
      ...
      static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end)
      {
         ...
         if (it + 2 < end && *it == '"' && *(it + 1) == '"' && *(it + 2) == '"') {
            ...
            while (it + 2 < end && !(*it == '"' && *(it + 1) == '"' && *(it + 2) == '"')) {
               if (*it == '\\') {
                  ...
                  case 'f':
                     value.push_back('\f');
                     break;
                  // YAML: Any other character is an error for escape sequences in basic strings
                  ...
                  case '\r': // part of CRLF, handle similar to \n
                     ...
                     break;
                  default:
                     // In YAML, an unknown escape sequence is an error.
                     // For simplicity here, we might just append them or flag an error.
                     // Glaze JSON parser often appends, let's be stricter for YAML.
                     ctx.error = error_code::syntax_error;
                     return;
                  }
               }
            ...
            // Bare string (not typical in YAML values, but handle it if necessary for some edge cases)
            // YAML spec does not allow bare strings for values. This path should ideally not be hit for valid YAML.
            // If it is, it's likely an error or a deviation.
            // For now, let's assume this is an error according to strict YAML.
            ctx.error = error_code::syntax_error;
            return;
            /*
            while (it != end && *it != '\n' && *it != '\r' && *it != '#') {
      ...
      static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end)
      {
         ...
         while (it != end) {
            ...
            if constexpr (emplace_backable<T>) {
               ...
               from<YAML, value_type>::template op<Opts>(value[index], ctx, it, end);
               ++index;
            }
      ...
      GLZ_ALWAYS_INLINE void parse_toml_object_members(T&& value, It&& it, End&& end, Ctx&& ctx, bool is_inline_table)
      {
         ...
         while (it != end) {
            ...
            if (!is_inline_table && *it == '[') {
               ...
               // This part might need to be more sophisticated for full YAML table support.
               skip_to_next_line(ctx, it, end);
               continue;
            }
            ...
            const auto index = decode_hash_with_size<YAML, std::decay_t<T>, HashInfo, HashInfo.type>::op(
               ...
               visit<N>(
                  [&]<size_t I>() {
                     if (I == index) {
                        ...
                        from<YAML, member_type>::template op<Opts>(member_obj, ctx, it, end);
                     }
                  },
                  index);
            ...
            if (is_inline_table) {
               if (*it == '}') {
                  ...
                  if (it != end && *it == '}') { // Trailing comma case like { key = "val", }
                     // This is allowed by YAML v1.0.0 for inline tables
            ...
            visit<N>(
               [&]<size_t I>() {
                  if (I == index) {
                     ...
                     if (!(path.size() - 1)) {
                        using member_type = std::decay_t<decltype(member_obj)>;
                        from<YAML, member_type>::template op<toml::is_internal_on<Opts>()>(member_obj, ctx, it, end);
                     }
                     else {
   ...
   template <read_supported<YAML> T, is_buffer Buffer>
   [[nodiscard]] inline error_ctx read_yaml(T& value, Buffer&& buffer)
   {
      context ctx{};
      return read<opts{.format = YAML}>(value, std::forward<Buffer>(buffer), ctx);
   }

   template <read_supported<YAML> T, is_buffer Buffer>
   [[nodiscard]] inline expected<T, error_ctx> read_yaml(Buffer&& buffer)
   {
      T value{};
      context ctx{};
      const error_ctx ec = read<opts{.format = YAML}>(value, std::forward<Buffer>(buffer), ctx);
      if (ec) {
   ...
   template <read_supported<YAML> T, is_buffer Buffer>
   [[nodiscard]] inline error_ctx read_file_yaml(T& value, const sv file_name, Buffer&& buffer)
   {
      context ctx{};
      ...
      return read<opts{.format = YAML}>(value, buffer, ctx);
   }
}
